...
class Order extends BaseModel
{
    ...
    {
        try {
            $order = 'INSERT INTO orders(product_id, email,bill_name, bill_address_line1, bill_address_line2, bill_city, bill_state,bill_country,bill_postal_code,ship_name,ship_phone,ship_address_line1, ship_address_line2, ship_city, ship_country, ship_postal_code,order_status, stripe_client_secret,email_structure_violation,email_domain_violation,email_fraud_violation,address_violation,order_date) 
                                        values(:productId,:email,:billName,:billLine1,:billLine2,:billCity,:billState,:billCountry,:billPostalCode,:shipName,:shipPhone,:shipLine1,:shipLine2,:shipCity,:shipCountry,:shipPostalCode,:orderStatus,:clientSecretKey,:emailStructure,:emailDomain,:fraudEmail,:invalidAddress,:orderDate) ';
            $this->prepare($order);
    ...
    {
        try {
            $orderUpdate = "SELECT orders.id,orders.email,orders.ship_address_line1 as shipAddressLine1,orders.ship_address_line2 as shipAddressLine2,orders.ship_city as shipCity,orders.ship_country as shipCountry,orders.ship_postal_code as shipPostalCode FROM orders where id=:id";
            $this->prepare($orderUpdate);
    ...
    {
        try {
            $orderUpdate = "UPDATE  orders set email=:email,ship_address_line1=:shipLine1,ship_address_line2=:shipLine2,ship_city=:shipCity,ship_country =:shipCountry,ship_postal_code=:shipPostalCode,email_structure_violation=:emailStructure,email_domain_violation=:emailDomain,email_fraud_violation=:fraudEmail,address_violation=:invalidAddress where id=:id";
            $this->prepare($orderUpdate);
